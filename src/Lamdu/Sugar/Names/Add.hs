{-# LANGUAGE GeneralizedNewtypeDeriving, TypeFamilies, TemplateHaskell, NoMonomorphismRestriction, TupleSections, FlexibleContexts #-}
...
p2nameConvertor nameType (P1Name (P1StoredName aName text) tagsBelow _) =
    ...
    _ -> id
p2nameConvertor nameType (P1Name (P1AnonName uuid) _ textsBelow) =
    case nameType of
    Walk.Tag -> error "TODO: Refactor types to rule this out"
    Walk.TypeVar ->
        do
            collides <- mkCollides textsBelow
            nameGen (NameGen.newTypeVarName (not . collides . DisplayText) uuid)
    Walk.TaggedVar -> nameGen (NameGen.existingName uuid)
...
mkCollides ::
    MonadReader P2Env f =>
    MMap DisplayText (Set T.Tag) -> f (DisplayText -> Bool)
mkCollides textsBelow =
    Lens.view p2TextsAbove <&>
    \textsAbove autoText ->
    Lens.has (Lens.ix autoText) textsBelow ||
    textsAbove ^. Lens.contains autoText
    ...
    do
        ...
        (newNameForm, env1) <-
            ...
            P1AnonName ctx ->
                ...
                <&>
                \collides ->
                NameGen.newName varInfo (not . collides . DisplayText) ctx
                <&> AutoGenerated
                & Lens.zoom p2NameGen
                & (`runState` env0)
